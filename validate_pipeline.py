...
def validate_position_counts(trade_log, equity):
    ...
    if issues:
        ...
        # Analyze the pattern
        diffs = [i['diff'] for i in issues]
        if all(d == diffs[0] for d in diffs):
            print(f"\n  📊 Pattern: All mismatches have the same difference of {diffs[0]}")
            if diffs[0] == -1:
                print("     → Reported count is consistently 1 higher than actual")
                print("     → Likely cause: Stock position being counted as a position")
...
def validate_expiry_pnl(trade_log, expiry_log):
    """Validate expiry P&L calculations."""
    print("\n" + "=" * 80)
    print("VALIDATION 7: Expiry P&L Analysis")
    print("=" * 80)
    
    total_expiry_pnl = expiry_log['pnl'].sum()
    total_expired = expiry_log['num_expired'].sum()
    avg_pnl_per_expiry = total_expiry_pnl / total_expired if total_expired > 0 else 0
    
    print(f"  Total expiry events: {len(expiry_log)}")
    print(f"  Total options expired: {total_expired}")
    print(f"  Total expiry P&L: ${total_expiry_pnl:,.2f}")
    print(f"  Average P&L per expiry: ${avg_pnl_per_expiry:.2f}")
    
    # Show best and worst expiry days
    best_day = expiry_log.loc[expiry_log['pnl'].idxmax()]
    worst_day = expiry_log.loc[expiry_log['pnl'].idxmin()]
    
    print(f"\n  Best expiry day: {best_day['date'].date()} (${best_day['pnl']:,.2f})")
    print(f"  Worst expiry day: {worst_day['date'].date()} (${worst_day['pnl']:,.2f})")
    
    # Count profitable vs unprofitable expiries
    profitable = len(expiry_log[expiry_log['pnl'] > 0])
    unprofitable = len(expiry_log[expiry_log['pnl'] < 0])
    neutral = len(expiry_log[expiry_log['pnl'] == 0])
    
    print(f"\n  Profitable expiries: {profitable} ({profitable/len(expiry_log)*100:.1f}%)")
    print(f"  Unprofitable expiries: {unprofitable} ({unprofitable/len(expiry_log)*100:.1f}%)")
    print(f"  Neutral expiries: {neutral}")
...
def main():
    ...
    validations.append(("Expiry P&L", validate_expiry_pnl(trade_log, expiry_log)))
