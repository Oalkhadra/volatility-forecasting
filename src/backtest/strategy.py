...
# Vectorized Black-Scholes and Delta function
def black_scholes_price_vectorized(
    ...
    if np.any(normal_case):
        sqrt_T = np.sqrt(np.where(normal_case, T, 1.0))
...
def calculate_margin_requirement(strike: float, num_contracts: float) -> float:
    """Calculate margin requirement for short option position."""
    return strike * abs(num_contracts) * 100 * MARGIN_RATE
...
class MispricingStrategy():
    ...
    def __init__(self, 
        ...
        Args:
            ...
            vrp_calculator: VarianceRiskPremiumCalculator for regime-based VRP adjustment.
            target_dte: Specific DTE to trade
        """
    ...
    def _get_vol_forecast(self, dte_days: float, current_date: datetime) -> float:
        """
        Get volatility forecast from the vol model.
        ...
        if isinstance(self.vol_model, (GARCHForecaster)):
            if self.vol_model.is_fitted:
                ...
                return sigma
            return 0.11  # Fallback if not fitted
    ...
    def _apply_risk_premium(self, sigma: float, current_date: datetime = None) -> float:
        ...
        Returns:
            Adjusted volatility estimate (IV)
        """
        # Use regime-based VRP (additive adjustment)
    ...
    def _vectorized_get_volatilities(self, df: pd.DataFrame, current_date: datetime) -> np.ndarray:
        """
        Get volatility forecast for all options.
        
        Args:
            df: DataFrame with options data
            current_date: Current date for vol forecast
            
        Returns:
            Array of volatilities (with VRP applied) for each option
        """
        # Get single volatility forecast for target DTE
        is_fitted = getattr(self.vol_model, 'is_fitted', True) 
        
        if is_fitted:
            sigma = self._get_vol_forecast(self.target_dte, current_date)
        else:
            print("Warning! Model not fitted, using fallback volatility of 0.11")
            sigma = 0.08  # Fallback if model not fitted
        
        # Apply VRP adjustment (IV ≈ RV + VRP)
        vrp = self.vrp_calculator.get_vrp_adjustment(pd.Timestamp(current_date))
        sigma_adjusted = max(sigma + vrp, 0.05)  # Floor at 5%
        
        return np.full(len(df), sigma_adjusted)

    def update_vol_model(self, current_date: datetime) -> None:
        """
        Update vol model with data available up to current_date.
        
        For GARCH: Refit with expanding window.
    ...
    def check_early_exits(self,
        ...
        for _, pos in list(portfolio.positions.items()):
            if pos.position_type != 'option':
                continue
            ...
            # Check if mispricing has converged to (near) fair value using asymmetric thresholds
            should_exit = False
    ...
    def generate_signals(self, 
        ...
        # Filter for mispricings above asymmetric thresholds
        ...
        # Filter for options not far OTM
        ...
        for _, row in df_mispriced.iterrows():
            ...
            # Check if we have capital capacity for at least 1 contract
